/**
@brief Integration tests for command-line interface functionality
*/
#include <filesystem>
#include <fstream>
...
class MathUtils {
  ...
  config_file << R"({
    "languages": {
      "cpp": {
        "library": "../build/bin/tree-sitter-cpp.so",
    ...
    "extract_directory": "integration_test_extract/",
...
void teardownIntegrationTest() {
  ...
  std::filesystem::remove_all("integration_test_extract");
  std::filesystem::remove(integration_config_file);
}

/**
@brief Tests end-to-end CLI generate command functionality

This test validates the requirement that the CLI must successfully execute
the 'generate' command to process source files and produce documentation output.
It tests the complete integration from command parsing to file generation.

Requirements tested:
- CesiumDocCLI must accept and process 'generate' command
- --config option must be recognized and used correctly
- Generate command must return success exit code (0)
- Integration between CLI parsing and documentation generation must work

Testing rationale: The generate command is the primary user-facing functionality,
so it must work reliably in real-world usage scenarios with actual config files.
*/
