/**
@brief Tests for documentation string parsing functionality
*/
...
/**
@brief Tests parsing of standard Javadoc/Doxygen-style block comments

This test validates the requirement that the docstring parser must correctly extract
and parse standard documentation comments, including description text, parameter
documentation, and return value descriptions.

Requirements tested:
- Parser must identify block comment delimiters in source code
- Description text must be extracted correctly
- @param tags must be parsed with parameter name and description
- @return tags must be parsed with return value description
- Multiple parameters must be handled correctly
- Extracted data must be stored in correct DocstringBlock fields

Testing rationale: Docstring parsing is fundamental to documentation generation -
if comments cannot be parsed correctly, the entire documentation system fails.
*/
...
/**
@brief Tests parsing of multiple documentation blocks in a single source file

This test validates the requirement that the parser must handle multiple docstring
blocks within the same source file, maintaining separation and correct association
of each block's content.

Requirements tested:
- Parser must find all docstring blocks in a file, not just the first
- Each block must be parsed independently with correct content
- Block count must match expected number of docstrings
- No cross-contamination between different blocks' content

Testing rationale: Real source files contain multiple documented functions/classes,
so the parser must handle multiple blocks reliably.
*/
