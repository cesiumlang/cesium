/**
 * @file ts_ast_parser.cpp
 * @brief Tree-sitter AST parsing utilities implementation
 */
#include "ts_ast_parser.h"
#include <cstring>

void DocAssociator::associateDocsWithNodes(std::vector<JavadocBlock>& javadoc_blocks,
                                           TSTree* tree, const std::string& content) {
  TSNode root = ts_tree_root_node(tree);

  for (auto& block : javadoc_blocks) {
...
TSNode DocAssociator::findFollowingDeclaration(TSTree* tree, TSNode root, SourceLocation javadoc_loc) {
  ...
  while (ts_query_cursor_next_match(cursor, &match)) {
    ...
    if (start_byte > javadoc_loc.byte_offset) {
      uint32_t distance = start_byte - javadoc_loc.byte_offset;
...
TSNode DocAssociator::getNameNode(TSNode node) {
  ...
  if (node_type == "function_definition") {
    // Look for function_declarator child, then identifier within it
    for (uint32_t i = 0; i < ts_node_child_count(node); i++) {
      TSNode child = ts_node_child(node, i);
      if (std::string(ts_node_type(child)) == "function_declarator") {
        // Now look for identifier in the function_declarator
